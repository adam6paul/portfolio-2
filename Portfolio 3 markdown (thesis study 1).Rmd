...
This only works with Hmisc on, but that contradicts my later use of summarize. 
# What's the better way of doing this?

```{r bivariate correlations}
correlations <- study1 %>%
        select(class_belong_comp4, school_belong_comp4, class_disclose_comp2, group_disclose_comp2, class_disclose_comp2, group_disclose_comp2)
...
```{r model creation}
model_1b <-   '  #Class Belonging direct effects
                class_belong_comp4 ~ a*class_disclose_comp2 + gender_nonbinary +                             gender_male_dummy + race_asian + race_black + race_hispanic +                                race_multiracial + class_know_prior + know_prior_group
                ...
                cdiscl_prompt_comp2 ~ gdiscl_prompt_comp2
...
```{r testing fit w/ int student status}
...
And so, we will run the model again, but this time dropping anyone who has no disclosure at all. While this does eliminate a key segment of data.

"appears that many people are in asychnronrous classes, but we don't know exactly what's going on."

Among those that have some opportunity to disclose. We know lots of people are not disclosing and have a wide range of belonging. Among those that do disclose...

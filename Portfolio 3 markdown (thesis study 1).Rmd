...
Several variable names were changed in excel. Specifically, the names of the gender responses were changed from a number to their actual gender identity (e.g., demog_gender_1 became demog_gender_female)
...
```{r}
summary(study1$class_belong_comp4, study1$school_belong_comp4)
...
As expected, class members known prior is a predictor of belonging and group members known is marginal.
...
model_1a <-   '  #Class Belonging direct effects
                class_belong_comp4 ~ a*class_disclose_comp2 + gender_nonbinary +                             gender_male_dummy + race_asian + race_black + race_hispanic +                                race_multiracial + class_know_prior + know_prior_group + demog_international
                
                class_belong_comp4 ~ b*group_disclose_comp2
                
                #School Belonging direct effects
                school_belong_comp4 ~ c*class_disclose_comp2 + gender_nonbinary +                            gender_male_dummy + race_asian + race_black + race_hispanic +                                race_multiracial + class_know_prior + know_prior_group + demog_international
...
This may be because people are mostly at a 1 in disclosure., so we want to look at folk with disclosure.
...
study1 %>%
        group_by(group_use) %>%
        summarize(freq=n())
#Most people are saying they don't use groups at all

#Graphing thie relationship between class disclosure and class belonging.
plot(study1$class_disclose_comp2, study1$class_belong_comp4)
```

The graph makes it clear that there's pretty wide variance for the people with no class disclosure that may be stifling the variance from appearing in the model.

```{r}
basic_correlations <- gather(study1, class_belong_comp4)
```
...
While the above model is good, it is washing out a lot of data from the class disclosure, as we only got responses for group disclosure from students who did meet in groups. As such, we're interested in the full class disclosure data set.
...
This means that class disclosure is getting washed out by the removal of the additional variables, or that it is being accounted for by group disclosure. Something worth looking at is whether     
